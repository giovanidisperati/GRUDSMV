---
layout: aula
title: "6. Atividade especial (a ser refatorada)"
...
>Aviso importante: O material teórico a seguir está sendo incorporado oficialmente a esta etapa da disciplina e fará parte de uma refatoração pela qual a Aula 05 passará brevemente (não deu tempo ainda!). Ele foi introduzido aqui temporariamente para fornecer a base conceitual de arquitetura e design de software que utilizaremos daqui em diante, servindo como leitura obrigatória para as próximas etapas práticas do nosso projeto. Nas próximas semanas essa seção deixará de existir, mas o material continuará disponível como parte integral da Aula 05 refatorada. 

Todo sistema de software começa com uma decisão, mesmo que implícita: como organizaremos o conhecimento do problema dentro do código? Essa decisão, que é, antes de tudo, uma decisão de arquitetura, vai muito além de escolher quais bibliotecas usar ou em quais pastas salvar os arquivos. 

Ela define como as responsabilidades serão distribuídas entre as partes do sistema, como as mudanças futuras serão absorvidas sem provocar colapso e, principalmente, como outros desenvolvedores (ou você mesmo, seis meses depois) vão conseguir ler o que foi escrito e entender a intenção por trás do código. 

Arquitetura de software é, em última instância, a arte de tornar intenções explícitas. Um sistema sem arquitetura deliberada não é neutro: ele simplesmente acumula as decisões implícitas de quem o escreveu sob pressão e cansaço. Com o tempo, essas decisões não-declaradas transformam o código em um labirinto onde cada mudança exige coragem. É uma situação em que a dívida técnica acumulada cobra juros crescentes.

Nesse sentido, existe uma tensão crescente no mercado de tecnologia que vale nomear diretamente: escrever código está se tornando uma habilidade cada vez menos exclusiva. Ferramentas como Copilot, Cursor, Claude, GPT e DeepSeek já são capazes de gerar funções, componentes e até módulos inteiros a partir de uma descrição em linguagem natural. As barreiras de entrada para produzir código funcional foram radicalmente reduzidas pela IA. A Shopify anunciou que o Copilot já escreve mais de 25% do seu código novo. A Amazon relatou que o CodeWhisperer reduz em até 57% o tempo de implementação de tarefas rotineiras.

E o que isso significa para você, que está se formando agora? Significa que o código em si, a linha digitada, o loop implementado, o componente renderizado, está se tornando cada vez mais uma commodity. 

O que não se commoditiza é a capacidade de entender por que uma estrutura é mais robusta do que outra, quando uma decisão arquitetural cria dívida técnica, e como expressar com clareza as fronteiras de um sistema para que ele escale sem virar um caos. A barreira continua sendo entender o problema real e tornar isso em uma base de código gerenciável e compreensível pela equipe. A diferença é que agora essas habilidades serão ainda mais importantes.

A IA pode escrever um useEffect para você. Ela não pode, por outro lado, bater o martelo se aquele useEffect deveria estar na tela, num hook customizado ou num serviço separado. Entender esse tipo de diferença é o que o mercado vai esperar cada vez mais daqui em diante.

É dentro desse contexto que esta aula se insere de forma estratégica no semestre. Vocês estão em meio aos Sprints de construção de um projeto mobile. E a tentação natural, especialmente com ferramentas de IA disponíveis, será sair implementando funcionalidades o quanto antes. Isso é o esperaad. Esta aula existe para inverter essa lógica: a implementação de código-fonte é subordinada às decisões de arquitetura, não o contrário. Por isso até mais importante do que escrever nossas linhas de código, é ser capaz de responder: onde vive a lógica de negócio? Quem é responsável por preparar os dados para a tela? Como a interface se comunica com os serviços externos sem criar dependências indevidas?

Essas perguntas têm respostas conhecidas e vamos abordá-las sob a ótica do ecossistema React Native que estamos lidando, para termos o ferramental necessário para expressá-las por meio da arquitetura e do design do código, não apenas do resultado que aparece na tela.

## **Modularização de código-fonte e Boas Práticas**
...
Nas últimas aulas, já tomamos decisões importantes nessa direção ao separar componentes de UI, telas, serviços e tipos em diretórios distintos. Mas separar em pastas é uma condição necessária, não suficiente. O que garante a saúde de longo prazo de um projeto é a disciplina de aplicar, dentro de cada pasta e em cada arquivo, um conjunto de práticas que tornam o código modular, coeso, fracamente acoplad e testável, independentemente do seu tamanho.

Conforme o projeto cresce, manter uma estrutura organizada é importante para a manutenibilidade. Já iniciamos essa organização por meio de diretórios, mas é necessário entender como avançar nesse sentido por meio da compreensão do Design e da Arquitetura.

Podemos citar como boas práticas, por exemplo: a **Separação de Componentes**, o uso de uma **Camada de Serviço**, a definição de **Funções Utilitárias** e a definição de **Tipos Globais**.

Nesse sentido, à medida que a aplicação cresce, precisamos reforçar a organização para garantir manutenção e escala.

Por exemplo, em nossa implementação, continuamos separando **componentes de interface reutilizáveis** (como o `PokemonCard`) dentro de `src/components`, enquanto **telas completas**, `PokedexScreen`, `PokemonDetailsScreen`, permanecem em `src/screens`.

Link para o repositório de código-fonte, caso queira consultar para entender mais concretamente a implementação: https://github.com/giovanidisperati/PokedexApp/tree/Aula05

Se uma tela ficar muito carregada, podemos dividi-la em subcomponentes, que podem "morar" numa subpasta própria dessa tela (exemplo: `src/screens/Pokedex/components`) ou, se forem realmente genéricos, subir para `src/components`. Um exemplo seria transformar a seção de detalhes do Pokémon em um `PokemonDetailsView` independente.
...
Cada camada tem uma responsabilidade clara e uma regra de dependência implícita: `components` e `screens` podem usar `services`, `types` e `utils`, mas o inverso jamais deve ocorrer, ou seja, um serviço nunca deve importar um componente de tela. Quando essa regra é violada, o acoplamento começa a corroer a estrutura.
...
Seguir essa divisão, com componentes reaproveitáveis, telas, serviços, utilitários e tipos globais, garante um código modular, facilita o trabalho em equipe e torna as próximas funcionalidades mais fáceis de integrar. Isso, contudo, diz respeito mais às decisões de "baixo nível": o design concreto e localizado de cada peça da aplicação. Essas decisões de design e as decisões de arquitetura não são camadas separadas e independentes, elas se influenciam mutuamente. A organização de pastas que acabamos de ver é, em grande medida, uma *consequência* de decisões arquiteturais mais profundas: quem pode depender de quem, onde vivem as regras de negócio, quem tem autoridade para mudar o estado da tela. É exatamente sobre essas decisões mais fundamentais que falaremos a seguir.

## **Arquiteturas Comuns em Aplicativos Móveis**
...
Em frameworks modernos como o React, a distinção entre View e Controller muitas vezes se torna turva. 
...
Vale deixar claro, antes de seguir em frente: **o MVC não é um padrão ruim**. Ele é um padrão com um contexto de uso bem definido. Em projetos pequenos, protótipos e provas de conceito, a estrutura adicional dos padrões que veremos a seguir pode ser um exagero que custa mais do que entrega. O problema do MVC não é conceitual, é que ele **não fornece mecanismos ativos para evitar** que a View e o Controller se fundam com o tempo, especialmente em ecossistemas como o React, onde o paradigma funcional e os hooks tornam essa fusão o caminho de menor resistência. Nos padrões seguintes, veremos justamente como resolver essa fragilidade estrutural.
...
Observe agora como o mesmo contador é implementado em MVP, e preste atenção especial ao que deixa de existir dentro da tela:
...
Nesse sentido, as **responsabilidades dos componentes** passam a ser:
...
Você acabou de ver o mesmo contador implementado em três arquiteturas diferentes. Agora é hora de consolidar esse entendimento na prática.

**Exercício rápido (~15 minutos):** Pegue o componente `CounterScreen_MVC.tsx` apresentado acima e refatore-o para o padrão **MVVM**, criando dois arquivos:
...
**Implementação adicional:** Adicione ao ViewModel um terceiro comando, `decrement`, e um estado derivado `isZero` que retorna `true` quando o contador está em zero. Use `isZero` na View para desabilitar o botão de decremento (`disabled={isZero}`). Observe como a View continua sem lógica condicional própria, ela apenas consome uma propriedade que o ViewModel calculou.